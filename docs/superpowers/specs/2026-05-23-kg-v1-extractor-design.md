...
3. **One concrete native plugin:** Go-via-tree-sitter, proving the contract handles a realistic structural extractor with non-trivial output (~1000 ops on a moderate codebase).
...
kg-extractor extract --plugin go-tree-sitter --input ./src --domain my-app --db ./kg.db

# External pipe (debugging or custom processing in between):
kg-extractor extract --plugin go-tree-sitter --input ./src --domain my-app \
...
```go
// plugins/go-tree-sitter/go.mod
module github.com/ggfarmco/kg/plugins/go-tree-sitter
...
require (
    ...
    github.com/smacker/go-tree-sitter v0.0.0-...               // CGO
)
```
...
- `make build-plugin-gots` — produces `bin/kg-extractor-go-tree-sitter` from the `plugins/go-tree-sitter/` module (CGO required).
...
```json
{
  "name": "go-tree-sitter",
  "version": "0.1.0",
  "description": "Extract Go code structure via tree-sitter",
  "runtime": "native",
  "executable": "kg-extractor-go-tree-sitter",
...
Fields:
- `input` — opaque to kg-extractor; plugin-specific meaning (for go-tree-sitter, an absolute path to a Go module/directory).
...
```json
{"op":"meta",        "args":{"plugin":"go-tree-sitter","version":"0.1.0","total_ops":1543}}
...
## tree-sitter-go Plugin

Native CGO binary `kg-extractor-go-tree-sitter`. Built with `CGO_ENABLED=1`. Links against `github.com/smacker/go-tree-sitter` and `github.com/smacker/go-tree-sitter/golang`.
...
│   └── go-tree-sitter/                    # NEW separate Go module
│       ├── go.mod                         # module github.com/ggfarmco/kg/plugins/go-tree-sitter
...
├── Makefile                               # +3 targets (build-extractor, build-plugin-gots, e2e)
...
- `plugins/go-tree-sitter/` (the producer; uses it to emit well-typed ops with `batch.Encoder`).
...
```makefile
.PHONY: build-extractor build-plugin-gots e2e test-all

build-extractor: build
	go build -o ./bin/kg-extractor ./cmd/kg-extractor

build-plugin-gots:
	CGO_ENABLED=1 go -C ./plugins/go-tree-sitter build -o ../../bin/kg-extractor-go-tree-sitter .

test-all: test
	go -C ./plugins/go-tree-sitter test ./...

e2e: build build-extractor build-plugin-gots
...
- **`plugins/go-tree-sitter/`**: Slug sanitization edge cases; param/return/receiver helpers on synthetic tree-sitter ASTs (where feasible — most decl extraction is integration-tested via golden). Runs as part of `make test-all`, not the default `make test`.
...
func TestExtractSelf(t *testing.T) {
    // 1. Build all three binaries (kg, kg-extractor, kg-extractor-go-tree-sitter).
    // 2. Lay out a tmp plugins dir with the go-tree-sitter manifest.
    // 3. Run: kg-extractor extract --plugin go-tree-sitter \
...
### Golden tests for go-tree-sitter

```
plugins/go-tree-sitter/testdata/golden/
├── 01-single-file/{input/, expected.jsonl}
├── 02-multi-package/{input/, expected.jsonl}
└── 03-with-methods/{input/, expected.jsonl}
```

Each subtest runs the plugin against `input/` and diffs stdout against `expected.jsonl`. Update via `go test -update`. Coverage:
...
| CGO build complexity for users | `make build` is unchanged (no CGO); CGO is opt-in via `make build-plugin-gots`. Plugin lives in its own module so its `go.sum` doesn't leak into kg's. README explains. |
...
7. Workspace setup: `plugins/go-tree-sitter/` as a separate Go module with its own `go.mod`/`go.sum`, `go.work` at repo root, `replace` directive for local dev.
8. `plugins/go-tree-sitter/` implementation — tree-sitter init, directory walker, package/file/decl extraction, imports, intra-package call graph, slug sanitization. Uses `github.com/ggfarmco/kg/batch` for op emission.
9. Golden tests for go-tree-sitter.
10. `e2e/extract_self_test.go` — wire everything together.
11. Makefile additions, README section on extractors.
